    ...
    : IVersionStrategy
{
    ...
    private IReadOnlyDictionary<ICommit, (IBranch B, IBranchConfiguration C)> GetCommitsWasBranchedFrom(
        IBranch branch, params IBranch[] excludedBranches)
    {
        Dictionary<ICommit, (IBranch B, IBranchConfiguration C)> result = new();
        ...
        foreach (var item in branchCommitDictionary.Keys)
        {
            ...
            if (result.ContainsKey(branchCommitDictionary[item]))
            {
                ...
                if ((branchConfiguration.IsMainBranch ?? Context.Configuration.IsMainBranch) == true
                    && result[branchCommitDictionary[item]].C.IsMainBranch == false)
                {
